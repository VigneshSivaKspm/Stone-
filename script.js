...
const firebaseConfig = {
    ...
    storageBucket: "legendarysnakegame.firebasestorage.app",
...
const mainBtnSound = new Audio('mainBtn.mp3');
const selfBiteSound = new Audio('selfBite.mp3');
let score = 0;
let direction = 'right';
...
if (!storedUsername) {
    // Prompt the user for their username if it doesn't exist
    const username = prompt("Enter your username:");
    if (username && username.trim() !== "") {
        localStorage.setItem('username', username);
        storeUsernameInFirebase(username);  // Store in Firebase
    } else {
        alert("Username is required!");
        // Optionally, you can prompt again if the user leaves the username blank
        // or provide them a way to re-enter it.
    }
} else {
    console.log("Welcome back, " + storedUsername);
    // If username exists, display it
    // You can also make use of the username in the game if needed
}

function storeUsernameInFirebase(username) {
    db.collection("users").add({
        username: username,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
    })
        .then((docRef) => {
            console.log("Username stored in Firebase with ID: ", docRef.id);
        })
        .catch((error) => {
            console.error("Error adding username to Firebase: ", error);
        });
}
